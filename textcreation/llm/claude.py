...
class claude(llm):
    ...
    async def get_completion_async(self, messages:dict, model:str="claude-opus-4-5-20251101", max_tokens:int=1024, temperature:float=0.8, timeout_seconds: Optional[int]=None):
    ...
    def get_completion_sync(self, messages:dict, model:str="claude-opus-4-5-20251101", max_tokens:int=1024, temperature:float=0.8):
        message = self.create_api_message(self.client, messages, model, max_tokens, temperature)
        return message.content[0].text
    
    async def get_completion_stream_async(self, messages:dict, model:str="claude-opus-4-5-20251101", max_tokens:int=1024, temperature:float=0.8, method=print):
        system = messages[0]
        messages = messages[1:]
    ...
    def get_completion_stream_sync(self, messages:dict, model:str="claude-opus-4-5-20251101", max_tokens:int=1024, temperature:float=0.8, method=print):
        system = messages[0]
        messages = messages[1:]
    ...
    def create_api_message(self, cli, messages:dict, model:str="claude-opus-4-5-20251101", max_tokens:int=1024, temperature:float=0.8):
